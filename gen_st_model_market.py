...
parser.add_argument('--data_dir',default="./market_rename/",type=str, help='./train_data')
parser.add_argument('--name', default='market', type=str, help='save model path')
...
def spatial_temporal_distribution(camera_id, labels, frames):
    ...
    result1 = {'spatial_temporal_avg':spatial_temporal_avg}
    result2 = {'distribution':distribution}
    scipy.io.savemat('model/'+name+'/'+'spatial_temporal_avg.mat',result1)
    scipy.io.savemat('model/'+name+'/'+'distribution.mat',result2)
    for i in range(8):
        for j in range(8):
            distribution[i][j][:]=distribution[i][j][:]/(sum_[i][j]+eps)
    result3 = {'distribution':distribution}
    scipy.io.savemat('model/'+name+'/'+'distribution_1.mat',result3)
...
distribution1 = np.squeeze(distribution[1,:,:])
plt.plot(distribution1)
plt.show()
# for i in range(0,8):
#     for j in range(0,8):
